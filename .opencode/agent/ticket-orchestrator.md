...
6. Round 2: **resume** the same sessions with their `task_id` (do not spawn
   fresh) and repeat steps 3–5, again in parallel, seeding each reviewer with
   the round-1 reports, so round 2 starts from the accumulated findings without
   reloading context.
...
- Reviewers report only within their own scope (see `docs/workflow.md`). Do not
  ask one to cover another's scope.
- Findings carry no severity labels; every finding is work. If a reviewer
  declines to stand behind a fix, require a named deferral target and reason.
